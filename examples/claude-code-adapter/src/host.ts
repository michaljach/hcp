...
import type {
  Harness, PermissionAsk, HarnessEvent,
} from "./harness.ts";
import type {
...
export class HcpHost {
  ...
  private resolve(
    ...
    const opt = optionId ? OPTIONS.find((o) => o.id === optionId) : undefined;
    ...
    s.harness.answerPermission(p.harnessRequestId, allow, text);
    ...
    switch (method) {
      case "initialize": {
        ...
        return {
          ...
          capabilities: {
            // Claude Code has no mid-turn steer; clients must queue a prompt instead.
            steer: false,
            interrupt: true, replay: true, snapshot: true, lease: false,
            fork: false, rollback: false, diff: true, terminal: false, push: false,
      ...
      case "session/steer":
        throw rpc(ERR.capabilityUnsupported,
                  "Claude Code has no mid-turn steer; queue a prompt instead");
